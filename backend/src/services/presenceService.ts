import { query } from "../repositories/dbRepository";
import { addEvent } from "./eventService";
import { getPolicy } from "./policyService";

const IDLE_AFTER = 60 * 1000;

export function startPresenceMonitor() {
  setInterval(async () => {
    const now = Date.now();
    const policy = getPolicy();
    const lostAfter = Math.max(1000, Number(policy.offline_seconds || 20) * 1000);
    const idleAfter = Math.min(IDLE_AFTER, lostAfter);
    const result = await query("SELECT * FROM devices");

    for (const d of result.rows) {
      const diff = now - Number(d.last_seen);

      if (diff < idleAfter) {
        await query("UPDATE devices SET state='ONLINE' WHERE id=$1", [d.id]);
        continue;
      }

      if (diff < lostAfter) {
        await query("UPDATE devices SET state='IDLE' WHERE id=$1", [d.id]);
        continue;
      }

      await query("UPDATE devices SET state='LOST' WHERE id=$1", [d.id]);

      const exist = await query(
        "SELECT 1 FROM alerts WHERE id=$1 AND message='Device not reporting' AND resolved=false",
        [d.id]
      );

      if (exist.rowCount === 0) {
        await query(
          "INSERT INTO alerts(id,message,time,acknowledged,resolved) VALUES ($1,'Device not reporting',$2,false,false)",
          [d.id, now]
        );
        await addEvent(d.id, "OFFLINE", "Device stopped reporting");
      }
    }
  }, 30000);
}
