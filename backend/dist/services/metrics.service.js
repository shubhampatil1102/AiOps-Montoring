...
async function checkCpuAnomaly(id, cpu) {
    const result = await (0, db_repository_1.query)(`SELECT cpu FROM metrics_history
    ...
    if (cpu > avg * 1.8) {
        ...
        await (0, db_repository_1.query)("INSERT INTO alerts(id,message,time,acknowledged,resolved) VALUES ($1,$2,$3,false,false)", [id, msg, Date.now()]);
        await (0, event_service_1.addEvent)(id, "CPU_ANOMALY", msg);
    }
}
async function ingestMetrics(body) {
    ...
    const now = Date.now();
    logger_service_1.Logger.info("FULL METRICS BODY:", body);
    await (0, db_repository_1.query)(`INSERT INTO devices (id,cpu,ram,time,last_seen,state,boot_time)
  ...
  `, [
    ...
    for (const issue of aiIssues) {
        await (0, suggestion_service_1.createSuggestion)(id, issue.type, issue.reason, issue.action, "AI Generated Fix");
    }
    await (0, db_repository_1.query)("INSERT INTO metrics_history (id,cpu,ram,time) VALUES ($1,$2,$3,$4)", [id, cpu, ram, now]);
    for (const p of processes) {
        await (0, db_repository_1.query)("INSERT INTO processes (device_id,name,cpu,ram,time) VALUES ($1,$2,$3,$4,$5)", [id, p.name, p.cpu, p.ram, now]);
    }
    ...
    if (cpu > policy.cpu_threshold) {
        ...
        const alert = await (0, db_repository_1.query)("INSERT INTO alerts(id,message,time,acknowledged,resolved) VALUES ($1,$2,$3,false,false) RETURNING time", [id, msg, now]);
        if (top) {
            const suggestionId = await (0, suggestion_service_1.createSuggestion)(id, "CPU_HIGH", `${top.name} using ${top.cpu}% CPU`, `Kill process ${top.name}`, `Stop-Process -Name "${top.name}" -Force`);
            await (0, db_repository_1.query)("UPDATE alerts SET suggestion_id=$1 WHERE time=$2", [suggestionId, alert.rows[0].time]);
        }
    }
    if (ram > policy.ram_threshold) {
        const top = processes.sort((a, b) => b.ram - a.ram)[0];
        const msg = `High RAM ${ram}%${top ? ` (${top.name})` : ""}`;
        await (0, db_repository_1.query)("INSERT INTO alerts(id,message,time,acknowledged,resolved) VALUES ($1,$2,$3,false,false)", [id, msg, now]);
        await (0, event_service_1.addEvent)(id, "RAM_HIGH", msg);
    }
}
async function attemptAutoHeal(id, alertType) {
    const rule = await (0, db_repository_1.query)("SELECT * FROM heal_rules WHERE alert_type=$1 AND auto_enabled=true LIMIT 1", [alertType]);
    if (!rule.rows[0])
        return;
    await (0, db_repository_1.query)(`INSERT INTO script_jobs(device_id,script,status,created_at)
     VALUES($1,$2,'PENDING',$3)`, [id, rule.rows[0].script, Date.now()]);
    await (0, db_repository_1.query)("UPDATE alerts SET auto_healed=true WHERE id=$1 AND resolved=false", [id]);
}
