...
async function findScriptLibrary() {
    return (0, db_repository_1.query)(`
...
async function findLibraryScript(scriptId) {
    return (0, db_repository_1.query)("SELECT script FROM script_library WHERE id=$1", [scriptId]);
}
async function createPendingScriptJob(deviceId, script) {
    return (0, db_repository_1.query)(`INSERT INTO script_jobs(device_id,script,status,created_at)
...
async function findRecentScriptJobs() {
    return (0, db_repository_1.query)(`SELECT id,device_id,status,output,error,created_at,started_at,finished_at
     FROM script_jobs
     ORDER BY id DESC
     LIMIT 50`);
}
...
async function findNextPendingJob(deviceId) {
    return (0, db_repository_1.query)(`SELECT id, script, timeout
     FROM script_jobs
...
async function markJobRunning(jobId) {
    return (0, db_repository_1.query)("UPDATE script_jobs SET status='RUNNING', started_at=$1 WHERE id=$2", [Date.now(), jobId]);
}
async function appendJobOutput(jobId, chunk) {
    return (0, db_repository_1.query)(`UPDATE script_jobs
