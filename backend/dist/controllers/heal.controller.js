...
async function getSuggestions(_, res) {
    const r = await (0, db_repository_1.query)(`SELECT *,
...
async function approveSuggestion(req, res) {
    const s = await (0, db_repository_1.query)("SELECT * FROM heal_suggestions WHERE id=$1", [req.params.id]);
    if (!s.rows[0])
        return res.sendStatus(404);
    const sug = s.rows[0];
    await (0, db_repository_1.query)("UPDATE heal_suggestions SET status='APPROVED' WHERE id=$1", [sug.id]);
...
async function rejectSuggestion(req, res) {
    await (0, db_repository_1.query)("UPDATE heal_suggestions SET status='REJECTED' WHERE id=$1", [req.params.id]);
    res.send({ ok: true });
}
async function getHealTimeline(_, res) {
    const result = await (0, db_repository_1.query)(`
    SELECT 
      id as job_id,
      device_id,
      script,
