...
const parseIssueKey_util_1 = require("../utils/parseIssueKey.util");
async function getIssues(_, res) {
    logger_service_1.Logger.info("GET /issues called");
    const result = await (0, db_repository_1.query)(`SELECT * FROM alerts ORDER BY time DESC LIMIT 100`);
...
async function resolveIssue(req, res) {
    const parsed = (0, parseIssueKey_util_1.parseIssueKey)(String(req.params.id));
    if (!parsed) {
        return res.status(400).send({ error: "Invalid issue id" });
    }
    await (0, db_repository_1.query)("UPDATE alerts SET resolved=true WHERE id=$1 AND time=$2", [parsed.alertId, parsed.alertTime]);
    res.send({ ok: true });
}
async function escalateIssue(req, res) {
    ...
    await (0, db_repository_1.query)("UPDATE alerts SET acknowledged=true WHERE id=$1 AND time=$2", [parsed.alertId, parsed.alertTime]);
    res.send({ ok: true });
}
async function remediateIssue(req, res) {
    ...
    await (0, db_repository_1.query)("UPDATE alerts SET auto_healed=true WHERE id=$1 AND time=$2", [parsed.alertId, parsed.alertTime]);
    res.send({ ok: true });
}
